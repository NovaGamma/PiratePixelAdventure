...
class level_button(button):
    ...
    def draw(self,rect):
        ...
        screen.blit(self.text,(self.x+10,self.y+(self.height//3)))    
...
def load(plank1):
    ...
    for level in levels_name:
        lvl_button.append(level_button(325,105,30+i*(325+15),30+j*(105+25),level.rstrip('.txt')))
        i+=1
        if i==4:
            j+=1
            i=0
...
if entities!=[]:#asking to save only if the level isn't empty
    ...
    fond.set_alpha(150)
    screen.blit(fond,(0,0))
    question=font.render("HAVE YOU ALREADY SAVED ?",0,(255,255,255))
